...
# ── Rarity ordering built from RarityTier.id ─────────────────────────────────
# Main tiers  : id × 100  →  common=100 … eternal=700
# Sub  tiers  : id × 10   →  festival=510, cartoon=710
...
_rate: dict[int, float] = {}  # per-user rate-limit store
...
async def _fetch_user_characters(user_id: int):
    ...
    Returns (None, None, None)  — user not in DB.
    Returns ([], {}, user_doc)  — harem empty.
    """
    ...
    # collection_mode filter / pre-sort
    ...
    # harem_sort (only when cmode didn't already impose its own order)
    if cmode == "all":
        if sort_field == "name":
        ...
        else:  # "rarity" default — common first … eternal last
            filtered = sorted(
                filtered,
    ...
    if isinstance(favs, list):
        for fav_id in favs:
            for c in unique_chars:
                if str(c.get("char_id") or c.get("id") or "") == str(fav_id):
                    return c
...
async def _build_harem_text(
    ...
    for anime, chars in grouped.items():
        flower = _ANIME_FLOWERS[hash(anime) % len(_ANIME_FLOWERS)]
        try:
...
# ══════════════════════════════════════════════════════════════════════════════
# Keyboard builder  —  clean prev/next only, no inline mode
# ══════════════════════════════════════════════════════════════════════════════
...
def _build_nav_markup(
    ...
    return IKM([
        [
            nb("🌸 prev", page - 1),
            IKB(f"❤️ {page + 1} / {total_pages} ❤️", callback_data="noop"),
            nb("next 🌸", page + 1),
        ],
        [
            IKB("🔎 filter rarity",  callback_data=f"filter:{user_id}"),
...
async def display_harem(
    ...
    try:
        ...
        if not unique_chars:
            empty_msg = (
                f"🔎 No <b>{escape(filter_rarity)}</b> souls in your collection yet!\n"
                "<i>Try a different filter 🌸</i>"
                if filter_rarity
                else (
                    "🌸 <b>Your harem is empty!</b> 🌸\n\n"
                    "ʚ♡ɞ  No souls collected yet  ʚ♡ɞ\n\n"
                    "<i>Claim characters when they spawn\n"
                    "by pressing ❤️ — they're waiting for you!</i>"
                )
            )
            if callback_query:
                try:   await callback_query.message.edit_text(empty_msg, parse_mode=enums.ParseMode.HTML)
        ...
        markup = _build_nav_markup(user_id, page, total_pages, filter_rarity)
...
async def cmd_harem(client, message: Message):
    ...
    if not (group_ok and channel_ok):
        text, markup = _join_card(
            uid, message.from_user.first_name or "there",
...
async def setfav_hint_cb(_, cb):
    ...
    await cb.answer(
        "💗 Use /setfav <character_id> to set your favourite cover!",
        show_alert=True,
    )
...
async def filter_cb(_, cb):
    ...
    try:
        await cb.message.edit_text(
            "🌸 <b>filter your harem by rarity</b> 🌸\n"
            "<i>choose a tier below ꕤ</i>",
...
# ══════════════════════════════════════════════════════════════════════════════
# /gallery  —  browse every character in the bot's database
...
async def cmd_gallery(client, message: Message):
    ...
    if rarity_filter and not get_rarity(rarity_filter):
        ...
        return await message.reply_text(
            f"❌ Unknown rarity <code>{escape(rarity_filter)}</code>.\n"
            f"<i>Valid: {valid}</i>",
            parse_mode=enums.ParseMode.HTML,
        )
...
async def _display_gallery(
    ...
    cover_img = None
    try:
        cover_img = random.choice(char_list).get("img_url") or None
    except Exception:
        pass
        ...
        try:
            if cover_img:
                await callback_query.message.edit_media(
                    InputMediaPhoto(
                        cover_img, caption=text, parse_mode=enums.ParseMode.HTML,
                    ),
                    reply_markup=markup,
                )
                return
