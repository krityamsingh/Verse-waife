...
# Shown as colored dots on every inline button to match the bot style in screenshot
TEAM_COLOR = {
    "hero":    "🔵",   # blue  — heroes
    "villain": "🔴",   # red   — villains
...
def _shop_keyboard(page: int = 0, uid: int = 0) -> IKM:
    """
    Shop grid — every fighter button prefixed with its team color dot:
    ...
    for i in range(0, len(chunk), 2):
        ...
        for key in chunk[i:i+2]:
            ...
            row.append(IKB(
                ...
                callback_data=f"dbshop_view:{uid}:{key}:{page}",
            ))
        rows.append(row)
    # Nav row
    nav = []
    if page > 0:
        nav.append(IKB("◀️ Prev", callback_data=f"dbshop_page:{uid}:{page-1}"))
    if start + per_page < len(keys):
        nav.append(IKB("Next ▶️", callback_data=f"dbshop_page:{uid}:{page+1}"))
    if nav:
        rows.append(nav)
    # Bottom quick-links
    rows.append([
        IKB("🥋 My Fighters", callback_data=f"dbshop_mine:{uid}"),
        IKB("⚡ Power Up",    callback_data=f"dbshop_pwrup:{uid}"),
    ])
    return IKM(rows)


def _owned_pick_keyboard(owned: list[str], fight_id: str, role: str, page: int = 0) -> IKM:
    """Pick keyboard — colored by team."""
    per_page = 6
    start    = page * per_page
    ...
    for i in range(0, len(chunk), 2):
        ...
        for key in chunk[i:i+2]:
            ...
            row.append(IKB(
                ...
                callback_data=f"dbpick:{fight_id}:{role}:{key}",
            ))
        rows.append(row)
    nav = []
    if page > 0:
        nav.append(IKB("◀️", callback_data=f"dbpick_page:{fight_id}:{role}:{page-1}"))
...
def _action_keyboard(fight_id: str, uid: int, transformed: bool) -> IKM:
    """
    Combat buttons — full-width rows like the screenshot bot style.
    Attack row, Special row, Transform row (if not yet transformed).
    """
    rows = [
        [IKB("⚔️  ──  A T T A C K  ──", callback_data=f"dbact:{fight_id}:{uid}:attack")],
        [IKB("💥  ──  SPECIAL MOVE  ──", callback_data=f"dbact:{fight_id}:{uid}:special")],
    ]
    if not transformed:
        rows.append([IKB("🌀  ══  T R A N S F O R M  ══", callback_data=f"dbact:{fight_id}:{uid}:transform")])
    return IKM(rows)
...
async def dbshop_view_cb(_, cq: CallbackQuery):
    ...
    if not already and char["price"] > 0:
        rows.append([IKB(
            ...
            callback_data=f"dbbuy:{uid}:{key}:{page}",
        )])
    elif not already:
        rows.append([IKB(
    ...
    rows.append([IKB(f"{dot} ◀️ Back to Shop", callback_data=f"dbshop_page:{uid}:{page}")])
    ...
    try:
        if video:
            await cq.message.edit_caption(caption=text, reply_markup=IKM(rows))
        else:
            await cq.message.edit_text(text, reply_markup=IKM(rows))
    except Exception:
        await cq.message.edit_text(text, reply_markup=IKM(rows))
    await cq.answer()
...
async def dbbuy_cb(_, cq: CallbackQuery):
    ...
    await cq.message.edit_text(
        ...
        reply_markup=IKM([[IKB(
            f"{dot} ◀️ Back to Shop",
            callback_data=f"dbshop_page:{uid}:{page_str}",
        )]]),
    )
...
async def dbupgrade_cmd(_, m: Message):
    ...
    buttons = IKM([
        [IKB(f"⚡  ══  Spend {next_t['cost']:,} Kakera  ══", callback_data=f"dbupg:{uid}:{cur}")],
...
async def wish_cmd(_, m: Message):
    ...
    # Full-width wish buttons styled like the screenshot
    buttons = IKM([[IKB(v["label"], callback_data=f"wish:{uid}:{k}")] for k, v in WISHES.items()])
    await m.reply(
        "🐉 **SHENRON AWAKENS!** 🐉\n\n"
...
async def dbhelp_cmd(_, m: Message):
    ...
    await m.reply(
        ...
        reply_markup=IKM([
            [IKB("🛒  ──  Open Shop  ──",   callback_data=f"dbshop_page:{uid}:0")],
            [IKB("🔵  Hero Fighters",        callback_data=f"dbshop_page:{uid}:0"),
             IKB("🔴  Villain Fighters",     callback_data=f"dbshop_page:{uid}:1")],
            [IKB("⚪  Neutral Fighters",     callback_data=f"dbshop_page:{uid}:3"),
             IKB("⚡  Power Up",             callback_data=f"dbupg:{uid}:cancel")],
            [IKB("🥋  My Collection",        callback_data=f"dbshop_mine:{uid}"),
             IKB("🐉  My Dragon Balls",      callback_data=f"dbshop_mine:{uid}")],
        ]),
    )
